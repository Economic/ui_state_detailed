...
date: "Data updated April 25, 2020"
output:
  html_document:
    theme: flatly
---
...
## Original data
This is data as reported from the sources below, with minimal re-coding. For various reasons, the total claims in these data will not necessarily match what the US DOL reports as the total for a given state.
```{r}
datatable(
...
## Adjusted data
This data takes the above and removes unclassified sectors and then scales IC totals to match the totals that states report weekly to the US DOL. The scaling preserves the classified industry distribution of claims within a state-week.
...
For most states the data are from state employment offices. Michigan's data is from the [W.E. Upjohn Institute](https://www.upjohn.org/research-highlights), which collected data from that state's employment office.

To do: provide detailed list of state data sources.
